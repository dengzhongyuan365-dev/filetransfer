...
- 图形界面 `lan-gui`
  ...
  - 支持粘贴剪贴板中的本地文件/文件夹和截图图片；截图图片传到对端后会自动写入对端剪贴板，方便继续粘贴。
...
默认搜索基于本机网卡的 UDP 广播，只能可靠覆盖同一个二层广播域。比如 `10.8.7.203/24` 和 `10.8.12.139/24` 从 IP 配置上看属于不同 `/24` 网段，普通广播通常不会被路由器转发过去，所以一边能发现另一边并不代表反向广播也一定能到。

如果两台机器处在可路由但不同广播域的网络里，可以在设置页的“额外发现网段”中配置明确的发现范围：

```txt
10.8.12.0/24
```

也可以配置多个条目，用逗号、分号或空格分隔：

```txt
10.8.12.0/24, 10.8.7.203
```

配置 `/24` 或单个 IP 时，程序会发送定点发现探测；配置更大的网段，例如 `10.8.0.0/16` 时，程序只按 `/24` 广播域发送定向广播，不会暴力扫描整个 `/16` 的每个主机。若网络设备禁用了跨网段定向广播，仍建议配置更精确的 `/24` 或单个 IP。
...
- `SettingsDialog`：封装接收目录、关闭行为和传输并发设置。
- `ReceiveHistoryDialog`：封装接收历史列表、打开目录和清空历史操作。
- `TransferCard`：封装单个传输任务的文件名、详情、速度、大小、状态和操作按钮。
...
- GUI 体验
  - 任务列表还有继续打磨空间。
  - 还没有任务详情页。
  - 历史记录和多设备队列已经可用，但还可以继续增强筛选、详情和批量操作。
