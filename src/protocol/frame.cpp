...
Result<bool> write_frame(const FileDescriptor& socket, const Frame& frame) {
    ...
    auto header_result = send_all(socket, header.data(), header.size());
    if (!header_result) {
    ...
    if (!frame.body.empty()) {
        ...
        auto body_result = send_all(socket, body, frame.body.size());
        if (!body_result) {
...
Result<Frame> read_frame(const FileDescriptor& socket) {
    std::array<char, header_size> header{};
    auto header_result = recv_exact(socket, header.data(), header.size());
    if (!header_result) {
    ...
    if (!frame.body.empty()) {
        ...
        auto body_result = recv_exact(socket, body, frame.body.size());
        if (!body_result) {
